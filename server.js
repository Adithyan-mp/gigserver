...
const io = socketIo(server, {
  cors: {
    origin: 'https://gigit-egbpfee6audkawdc.canadacentral-01.azurewebsites.net', // Fixed frontend URL
    methods: ["GET", "POST"],
    credentials: true,
  },
});
...
app.use(express.static(path.join(__dirname, "build"))); // Make sure the 'build' folder is in the root directory
...
app.post("/send-registration-email", async (req, res) => {
  ...
  try {
    ...
    const registrationLink = role === "seeker"
      ? `https://gigit-egbpfee6audkawdc.canadacentral-01.azurewebsites.net/seekerRegistration?token=${token}`
      : `https://gigit-egbpfee6audkawdc.canadacentral-01.azurewebsites.net/providerRegistration?token=${token}`;
